...
the paper with the conclussions and a discussion about our future 
plans in Section~\ref{sec:conclussions}.
...
an online community of people that share a common interest. The goal of 
SIOC is to interconnect these online communities. Other key concepts into 
the ontology are \textsf{sioc:User} and \textsf{sioc:Post}, which model 
respectively the members of the communities and the content they produce.
...
Each one fulfills a concrete purpose into the global purposes that the 
...
to the SIOC ontology. Parsing a mailing list it isn't a trivial task 
...
trying to resolve the possible conflicts found using several heuristic 
algorithms. (FIXME: this paragraph is not clear, rewrite it)
