...
\begin{itemize}
  \item To add XHTML markup using a syntax for layering RDF like RDFa \cite{Birbeck2006}.
	This new output format could publish under the same URI than the RDF files,
	and using content negotiation (FIXME: cite) configure the HTTP server (Apache or 
	other similar) to serve each according with the mime type accepted by the
	user agent.
