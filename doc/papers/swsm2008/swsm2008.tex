...
the FOAF and SIOC vocabularies are introduced in the context of appling
...
In the rest of the paper, we describe methods to create a significative
...
inferrence engines to obtain new knowledge.
...
human consumption, but also as gzipped mailboxes splitted by
month and mailing list.

A simple shell script was run to fetch the mailboxes, unpack and concat them
into a single mailbox for each mailing list. A subset of these files
(FIXME: how many lists??) were provided as input data to SWAML. The
