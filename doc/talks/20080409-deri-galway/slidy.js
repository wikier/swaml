...
var lin = null;       // table of contents
...
var strings_es = {
  ...
  "Table of Contents":"Tabla de Contenidos",
  "Table of Links":"Tabla de Enlaces",
...
var strings_nl = {
  ...
  "help?":"Help?",
  "contents?":"Inhoud?",
  "table of contents":"inhoudsoverzicht",
  "Table of Contents":"Inhoudsoverzicht",
...
function startup()
{
   ...
   if (slides.length > 0)
   {
      ...
      if (slidenum > 0)
      {
         setVisibilityAllIncremental("show");
      ...
      {
         lastShown = null;
         setVisibilityAllIncremental("oculto");
...
function keyDown(event)
{
    ...
    else if (key == 37 | key == 8) // Left arrow
    {
       previousSlide(!event.shiftKey);
       return cancel(event);
    }
    else if (key == 38 && !jarl) // Left arrow
    {
       previousSlide(!event.shiftKey);
       return cancel(event);
    }
    ...
    else if (key == 40 && !jarl) // Right arrow
    {
       nextSlide(!event.shiftKey);
       return cancel(event);
    }
    ...
    else if ( key == 67)  // C for table of contents
    {
       if (toc)
         showTableOfContents();
    ...
    else if (key == 84)  // T for table of links
    {
       if (lin)
         showTableOfLinks();
...
function showTableOfLinks()
{
  if (lin)
  {
    if (lin.style.visible != "visible")
    {
      lin.style.visible = "visible";
...
function hideTableOfLinks()
{
  if (lin)
  {
    lin.style.visible = "hidden";
...
// called onkeydown for lin entry
function gotoLinEntry(event)
{
  var key;
...
// crear lista de enlaces totales
function tableOfLinks()
{
  var lin = document.createElement("div");
  addClass(lin, "lin");

  var heading = document.createElement("div");
  addClass(heading, "lin-heading");
  heading.innerHTML = "Table of Links".localize();
  ...
  lin.appendChild(heading);
  var previous = null;
  var enlaces = document.getElementsByTagName("a");
  for (var i = 0; i < enlaces.length-10; ++i)
  {
    var num = document.createTextNode((i + 1) + ". ");
    lin.appendChild(num);

    var a = document.createElement("a");
    a.setAttribute("href", enlaces[i].href);
    a.setAttribute("class", enlaces[i].className);

    var name = document.createTextNode(enlaces[i].innerHTML);
    a.appendChild(name);
    a.onclick = gotoEntry;
    a.onkeydown = gotoLinEntry;
    a.previous = previous;
  ...
  lin.style.zIndex = "300";
  lin.style.width = "60%";
  lin.style.maxWidth = "30em";
  lin.style.height = "30em";
  lin.style.overflow = "auto";
  lin.style.top = "auto";
  lin.style.right = "auto";
  lin.style.left = "4em";
  lin.style.bottom = "4em";
  lin.style.padding = "1em";
  lin.style.background = "rgb(240,240,240)";
  lin.style.borderStyle = "solid";
  lin.style.borderWidth = "2px";
  lin.style.fontSize = "60%";

  document.body.insertBefore(lin, document.body.firstChild);
  return lin;
}
